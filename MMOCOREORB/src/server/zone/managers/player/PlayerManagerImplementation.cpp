...
void PlayerManagerImplementation::sendPlayerToCloner(CreatureObject* player, uint64 clonerID, int typeofdeath) {
	...
	if(ghost->getJediState() >= 2) {
		...
		int xpLoss = (int)(jediXpCap * -0.01);
		int curExp = ghost->getExperience("jedi_general");

		int negXpCap = -100000; // Cap on negative jedi experience
	...
	for (int i = 0; i < threatMap->size(); ++i) {
		...
		if (attacker->isPet()) {
			...
			for (int j = 0; j < entry->size(); ++j) {
				...
				//Jedi experience doesn't count towards combat experience, and is earned at 50% the rate of normal experience
...
void PlayerManagerImplementation::enhanceCharacter(CreatureObject* player) {
	...
	message = message && doEnhanceCharacter(0x98321369, player, medicalBuff, medicalDuration, BuffType::MEDICAL, 0);
